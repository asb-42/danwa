...
async def start():
    settings = await cl.ChatSettings([
        cl.input_widget.Select(id="profile", label="LLM-Profil", values=list(PROFILES.keys()), initial_value="local_lm_studio"),
        cl.input_widget.Slider(id="max_rounds", label="Max. Runden", min=1, max=5, step=1, initial=3),
        cl.input_widget.Slider(id="threshold", label="Konsens-Schwelle", min=0.5, max=1.0, step=0.05, initial=0.75),
        cl.input_widget.Checkbox(id="enable_fact_check", label="Web-Validierung aktivieren", value=True),
        cl.input_widget.Checkbox(id="enable_memory", label="Präzedenz-Speicher aktivieren", value=True)
    ]).send()
    ...
    settings_extended = await cl.ChatSettings([
        ...
        cl.input_widget.Checkbox(id="enable_fact_check", label="Web-Validierung aktivieren", value=True),
        cl.input_widget.Checkbox(id="enable_memory", label="Präzedenz-Speicher aktivieren", value=True),
        cl.input_widget.Select(id="variant", label="Prompt-Variante", values=variant_opts + ["auto"], initial_value="auto")
    ]).send()
    
    cl.user_session.set("settings", settings_extended)
    
    # Dashboard-Trigger hinzufügen
    start_actions = [cl.Action(id="open_dash", label="📊 Dashboard öffnen", value="init", description="Sitzungsverwaltung")]
    await cl.Message(content="🤝 Multi-Agent Debatten-System bereit.", actions=start_actions, author="System").send()
...
async def main(message: cl.Message):
    ...
    if message.elements:
        await cl.Message(content=f"📄 Verarbeite {len(message.elements)} Dokument(e)...", author="System").send()
    ...
    with cl.Step(name="Debatte", type="run") as run_step:
        state = await engine.run(context, progress_callback=progress, variant_override=variant)
    ...
    await cl.Message(
        ...
        author="System", type="info"
    ).send()

    # Füge Variante zur Ausgabe hinzu:
    variant_info = f"`{state.used_variant or variant or 'auto'}`" if state.used_variant or variant else "`auto`"
    ...
    await cl.Message(content="📄 Vollständiger Audit-Trace:", elements=elements, author="System").send()
    ...
    await cl.Message(
        ...
        elements=[
            cl.File(name=report_path_docx.name, path=str(report_path_docx), display="inline"),
            cl.File(name=report_path_pdf.name, path=str(report_path_pdf), display="inline")
        ],
        author="System"
    ).send()
...
async def handle_action(action: cl.Action):
    ...
    if action.id == "open_dash":
        ...
        await cl.Message(content=f"✅ Bereinigung abgeschlossen. {deleted_db} DB-Einträge entfernt.", author="System").send()
        await render_dashboard(db, 0, filt)
    ...
    # Session-Aktionen
        ...
        if trace_file.exists():
            await cl.Message(content=f"📄 Trace: `{action.value[:8]}...`", 
                             elements=[cl.Text(name=trace_file.name, path=str(trace_file), display="inline")], 
                             author="Trace").send()
        else:
        ...
        await cl.Message(content=f"✅ Session `{sid[:8]}...` vollständig gelöscht.", author="System").send()
        await render_dashboard(db, page, filt)
        ...
        if reports:
            elements = [cl.File(name=r.name, path=str(r), display="inline") for r in reports]
            await cl.Message(content=f"📥 Reports: `{sid[:8]}...`", elements=elements, author="Export").send()
        else:
            await cl.Message(content=f"⚠️ Kein Report gefunden. Bitte erneut generieren.").send()