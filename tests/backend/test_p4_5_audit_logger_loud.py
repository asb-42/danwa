...
class TestInsertFailureIsNonFatal:
    ...
    def test_dead_connection_does_not_raise(
    ...
    def test_cached_connection_is_dropped_on_failure(
...
class TestPerSessionErrorThrottling:
    ...
    def test_first_failure_per_session_logs_error(
        self, audit: AuditLogger, caplog: pytest.LogCaptureFixture
    ) -> None:
        ...
        errors = [
            ...
            if r.levelno == logging.ERROR and "sess-D" in r.getMessage()
        ]
        assert len(errors) == 1, [r.getMessage() for r in caplog.records]

    def test_subsequent_failures_drop_to_debug(
        self, audit: AuditLogger, caplog: pytest.LogCaptureFixture
    ) -> None:
        ...
        errors = [
            r
            for r in caplog.records
            if r.levelno == logging.ERROR and "sess-E" in r.getMessage()
        ]
        assert len(errors) == 1, [r.getMessage() for r in caplog.records]
    ...
    def test_different_sessions_each_get_their_own_error(
        ...
        error_sessions = {
            r.getMessage().split("for session ", 1)[1].split(";")[0]
            for r in caplog.records
...
class TestCloseFailureIsLoud:
    ...
    def test_close_failure_logs_warning(
        self, audit: AuditLogger, caplog: pytest.LogCaptureFixture
    ) -> None:
        ...
        with caplog.at_level(logging.WARNING, logger="backend.workflow.audit_logger"):
            audit.close()
        assert any(
            r.levelno == logging.WARNING and "closing audit logger" in r.getMessage()
            for r in caplog.records
