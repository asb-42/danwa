...
```bash
cp deploy/.env.example deploy/.env  # 编辑 JWT_SECRET_KEY
docker compose up -d                # 启动所有服务
# 使用 Celery worker 进行并行辩论：
...
- **统一反馈系统** — 工作流运行期间的实时状态栏、活动日志面板、分类错误显示和节点执行指示器
...
- **工作流模板** — 常见用例的预构建工作流模板
- **每智能体 LLM 参数** — 在蓝图中为每个智能体单独覆盖 temperature、top_p、top_k、frequency_penalty、presence_penalty
- **对比和回放视图** — 比较辩论会话并通过时间轴导航回放过去执行
- **多用户认证** — 基于 JWT 的认证，支持基于角色的访问控制（管理员/编辑者/查看者）
- **多租户架构** — 隔离的租户，支持案例、标签和配额
- **BYOK（自带密钥）** — 每用户 LLM API 密钥覆盖
- **事务性起草** — 结构化文档创建，包含 Builder、Pragmatist 和 Angel's Advocate 节点
- **Angel's Advocate** — 建设性倡导工作流节点
- **Kitsune 智能体工具** — 6 个只读工具，用于通过助手进行系统查询
- **速率限制** — 使用 slowapi 的可配置每端点速率限制
- **Prometheus 指标** — /metrics 端点用于监控
- **结构化日志** — 通过 structlog 在生产环境中输出 JSON 日志
- **可选 Celery 任务队列** — 使用 Redis 进行并行辩论执行
- **Docker 部署** — 生产就绪的 Dockerfile 和 docker-compose
...
│   │       ├── cases.py         # 案例管理
│   │       ├── tags.py          # 标签管理
│   │       ├── case_scoped.py   # 案例范围端点
│   │       └── user_keys.py     # 用户 API 密钥管理
...
│   │   ├── case.py          # 案例模型
│   │   ├── tag.py           # 标签模型
│   │   ├── membership.py    # 成员关系模型
│   │   └── transactional.py # 事务性起草模型
│   ├── workflow/
│   │   ├── debate_graph.py   # LangGraph 状态机构建器
│   │   ├── nodes.py          # 节点函数（initialize、run_agent 等）
│   │   ├── nodes/            # 专用工作流节点
│   │   │   ├── angels_advocate_nodes.py  # Angel's Advocate 工作流
│   │   │   ├── builder_nodes.py          # Builder 工作流节点
│   │   │   └── pragmatist_nodes.py       # Pragmatist 工作流节点
...
│   │   ├── audit.py         # 审计事件记录
│   │   ├── user_store.py    # 用户存储
│   │   ├── tenant_store.py  # 租户存储
│   │   ├── case_store.py    # 案例存储
│   │   ├── tag_store.py     # 标签存储
│   │   ├── membership_store.py # 成员关系存储
│   │   └── user_key_store.py # 用户密钥存储（BYOK）
│   ├── repositories/
...
├── docker-compose.yml           # 多服务 Docker Compose
├── deploy/                      # 部署配置
...
| 变量 | 描述 |
|------|------|
| `DANWA_JWT_SECRET_KEY` | JWT 令牌签名密钥 |
| `DANWA_AUTH_ENABLED` | 启用/禁用认证（默认：`false`） |

### 速率限制

| 变量 | 描述 |
|------|------|
| `DANWA_RATE_LIMIT_ENABLED` | 启用/禁用速率限制（默认：`false`） |
| `DANWA_RATE_LIMIT_DEFAULT` | 默认速率限制（例如 `60/minute`） |

### Redis / Celery

| 变量 | 描述 |
|------|------|
| `DANWA_REDIS_URL` | Redis 连接 URL（例如 `redis://localhost:6379/0`） |
| `DANWA_CELERY_ENABLED` | 启用 Celery 任务队列进行并行辩论（默认：`false`） |

### Prometheus

| 变量 | 描述 |
|------|------|
| `DANWA_PROMETHEUS_ENABLED` | 启用 Prometheus 指标端点（默认：`false`） |
...
**重要说明**：PaddlePaddle 3.3.0+ 存在已知的 PIR 与 OneDNN 兼容性问题，会导致 OCR 崩溃。版本约束 `<3.3.0` 确保 OCR 操作稳定。详见 [ADR-2024-05-12](docs/adr/2024-05-12-paddlepaddle-downgrade.md)。
...
*Danwa v2.2.0 | 基于 FastAPI + LangGraph + LiteLLM + Svelte 5 + @xyflow/svelte 构建*
